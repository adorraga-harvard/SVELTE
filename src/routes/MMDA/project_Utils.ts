...
export function findRouteData(slugString: string | undefined) {
    const result = { currentRoute: null as Route | null, agencyRoot: null as Route | null, breadcrumbs: [{ text: 'Metro Manila Development Authority', href: '/MMDA', icon: 'Home' }] };
    if (!slugString) {
        if (config && config.length > 0) {
            result.currentRoute = config[0]; result.agencyRoot = config[0];
            result.breadcrumbs.push({ text: config[0].navTitle || config[0].title, href: '/SQP/' + config[0].fs_path, icon: null });
        }
        return result;
    }
    const segments = slugString.split('/');
    let currentItems: Route[] | undefined = config;
    let currentPath = '/MMDA';
