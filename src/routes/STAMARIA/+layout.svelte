<script lang="ts">
  ...
  $: sidebarNodes = filterTree(navigationTree, searchTerm);

  function countNodes(nodes: NavNode[]): number {
    return nodes.reduce((count, node) => count + 1 + countNodes(node.children ?? []), 0);
  }

  $: totalNavNodeCount = countNodes(navigationTree);
      ...
      <div class="navbar px-4">
        <div class="navbar-start">
          ...
          <a href={basePath} class="flex items-center gap-2 text-lg font-bold transition-opacity hover:opacity-80 lg:hidden">
            ...
            <span class="hidden md:inline">{@html FULL_NAME}</span>
          </a>
        </div>
