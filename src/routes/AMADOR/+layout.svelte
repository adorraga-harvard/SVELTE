<script lang="ts">
  ...
  $: totalSidebarNodeCount = countNodes(sidebarSourceTree);
  	...
  	function handleImageError(e: Event & { currentTarget: HTMLImageElement }) {
		// This line is crucial to prevent an infinite loop if the
		// fallback image itself is also missing.
		e.currentTarget.onerror = null;

		// Set the src to the default/fallback image
		e.currentTarget.src = '/logos/project.png';
	}
    ...
    <header class="sticky top-0 z-30 w-full shadow-md bg-base-300/90 backdrop-blur-sm">
      <div class="navbar px-4">
        ...
        <div class="navbar-center hidden lg:flex">
          <NavBar links={navigationTree} variant="tabs" basePath={basePath} />
        </div>
      ...
      <ul class="menu text-base-content flex-1 overflow-y-auto project-tree-lines project-util-scrollbar p-4 pt-0">
        {#if activeTopLevelNode}
          {#if sidebarNodes.length > 0}
            <li class="project-menu-title"><span>{activeTopLevelNode.navTitle}</span></li>
